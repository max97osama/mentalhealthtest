...
export default function ResultsPageAr({ result }: Props) {
  const v = verdictConfig[result.verdict]
  const flagged = result.flagged_conditions || []

  return (
    <>
      <Head><title>نتائجك — MindCheck</title></Head>
      <Navbar lang="ar" />
      ...
      <main dir="rtl" style={{ maxWidth: '740px', margin: '0 auto', padding: '48px 20px 80px' }}>
        ...
        <div style={{ background: v.bg, border: `2px solid ${v.border}`, borderRadius: '16px', padding: '36px 32px', textAlign: 'center', marginBottom: '48px' }}>
          ...
          <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.7rem', color: v.color, marginBottom: '14px' }}>{v.label}</h2>
          <p style={{ color: '#9d8fc0', fontSize: '0.95rem', lineHeight: 1.8, maxWidth: '500px', margin: '0 auto' }}>{v.sub}</p>
        </div>

        <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.5rem', color: '#e8e0f5', marginBottom: '24px' }}>تفاصيل الحالات</h2>
              ...
              <div key={condition} className="card" style={{ padding: '20px 24px' }}>
                ...
                {isFlagged && (
                  <div style={{ marginTop: '10px' }}>
                    <Link href={`/ar/conditions/${condition}`} style={{ fontSize: '0.8rem', color: '#a78bfa', textDecoration: 'none' }}>
                      تعرف على {info.nameAr} ←
                    </Link>
                  </div>
                )}
              </div>
